...
// File identifier string: used for debug output
	private static String FILE = "SmeAddress";
	...
	public SmeAddress(int ton, int npi, String addr)
	{
		...
		if(addr == null || addr.length() > 20)
			throw new SMPPException("Destination address cannot be null and must be < 21 chars.");
		else
	...
	public SmeAddress(String addr)
	{
		...
		if(addr == null || addr.length() > 20)
			throw new SMPPException("Distribution list name cannot be null and must be < 21 chars.");
		else
			this.addr = new String(addr);
	}
		...
		throws IOException
	{
		...
		if(id)
		{
			...
			if(x == 1)
				...
				throw new SMPPException("Input stream does not contain a SmeAddress struct.");
		}
	...
	public void writeTo(OutputStream out)
	{
		...
		{
			throw new SMPPException("Error writing SmeAddress packet to output stream");
		}
	}
	...
	static void makeNameTables()
	{
		...
		{
			try { o = f[loop].get(null); }
			catch(NullPointerException x) { continue; }
			catch(IllegalAccessException x) { continue; }
