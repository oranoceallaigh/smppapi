...
// File identifier string: used for debug output
	private static String FILE = "SmppConnection";
	...
	public static final int		MINOR_VERSION = 0x00;
		...
		throws IOException
	{
		...
		if(r.getSeqNo() < seqNo-1)
			throw new SMPPException("Sequence numbering invalid.  Current=" + String.valueOf(seqNo-1) + ", Packet value=" + r.getSeqNo());
		...
		throws IOException
	{
		...
		try { rq = (SMPPRequest)inTable.get(key); }
		catch(ClassCastException cx)
			{ rq = null; }
		if(rq == null)
			throw new SMPPException("There was no request with sequence no. " + key);
		else if(rq.isAckd())
			throw new SMPPException("The packet with sequence " + key + " is already acknowledged.");
	...
	public void force_unbind()
	{
		if(!unbinding)
		{
			Debug.d(this, "force_close", "Force tried before normal unbind.", Debug.DBG_2);
			throw new SMPPException("Please Try a normal unbind before forcing one.");
		}

		Debug.d(this, "force_unbind", "Attempting to force the connection shut.", Debug.DBG_4);
		try
		{
			// The thread must DIE!!!!
	...
	public void reset()
	{
		if(bound || link.isConnected())
		{
			Debug.d(this, "reset", "Reset failed. bound="+bound+", link="+link.isConnected(), Debug.DBG_3);
			throw new SMPPException("Cannot reset connection while bound or connected.");
		}
		if(rcvThread.isAlive())
		{
	...
	public void notifyObservers(Object b)
	{
		if(! (b instanceof SMPPPacket))
			throw new IllegalArgumentException("Notify Observers needs an SMPPPacket class.");
	...
	public void run()
	{
		...
		while(true)
		{
			try
			{
				try { pak = SMPPPacket.readPacket(in); }
				catch(SMPPException ix)
				{
					/* Don't mind this.  Just try and read another one.. */
					Debug.d(this, "run", "Smpp Exception trying to read a packet."+ix.getMessage(), Debug.DBG_3);
					pak = null;
					continue;
				}
				...
				{ 
					...
					Debug.d(this, "run", "EOFException in thread. "+ex.getMessage(), Debug.DBG_3);
					bound = false;
					try { link.close(); }
					catch(IOException ix) { }
				...
				catch(IOException ix)
				{
					...
					Debug.d(this, "run", "IOException in thread"+ix.getMessage(), Debug.DBG_3);
					bound=false;
					try { link.close(); }
				...
				Debug.d(this, "run", "Packet recv:"+pak.getClass().getName(), Debug.DBG_2);
				...
				id = pak.getCommandId();
				if(!bound
				&& (   id == SMPPPacket.ESME_BNDTRN_RESP
				...
				if(ackDeliverSm && id == SMPPPacket.SMSC_DELIVER_SM)
				{
					...
					sendResponse(dr);
					Debug.d(this, "run", "Ack'd deliver_sm #"+dr.getSeqNo(), Debug.DBG_3);
				}
				...
				if(ackQryLinks && id == SMPPPacket.ESME_QRYLINK)
				{
					...
					sendResponse(el);
					Debug.d(this, "run", "Ack'd enquire_link #"+el.getSeqNo(), Debug.DBG_3);
				}

				// Tell all the observers about the new packet
				Debug.d(this, "run", "Notifying observers of new Packet", Debug.DBG_4);
				notifyObservers(pak);
			}
			catch(IOException x)
			{
				Debug.d(this, "run", "IOException: "+x.getMessage(), Debug.DBG_1);
				throw new SMPPException("I/O Error in listener." + x.toString());
			}
		} // end while
	} // end run()
}
