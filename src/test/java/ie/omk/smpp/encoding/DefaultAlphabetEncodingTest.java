 ...
 */
@Test
public class DefaultAlphabetEncodingTest {

    private final DefaultAlphabetEncoding encoding =
        new DefaultAlphabetEncoding();
    
    public void testDefaultAlphabet() {
        // 128 code points in the base table, and 2 bytes for each character
        // in the extended table. (The alphabet string has a space in the
        // place where the escape char would be).
        final int expectedEncodedLength = 128 + (2 * 9);
        ...
        assertEquals(encoded.length, expectedEncodedLength);
        assertEquals(decoded, alphabet);
        
        for (int i = 0; i < 128; i++) {
            if (i == DefaultAlphabetEncoding.EXTENDED_ESCAPE) {
                continue;
            }
            assertEquals(encoded[i], (char) i);
        }
    }
