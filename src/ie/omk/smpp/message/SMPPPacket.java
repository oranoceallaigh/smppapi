...
// File identifier string: used for debug output
	private static String FILE = "SMPPPacket";
	 ...
	 */
	static final String[] GsmStrings = {"No error",
...
/*
		...
		ERR_SMSC_SMPP_RPREDEFMSGNOTFND	= SMSCBASE+ESME_RPREDEFMSGNOTFND,
	...
	public SMPPPacket(int id, int seqNo)
	{
		...
		{
			Debug.d(FILE, "<init>", "Bad sequence no: "+seqNo, Debug.DBG_1);
	...
	public SMPPPacket(InputStream in)
	{
		...
		{
			if(Debug.dbg)
				Debug.d(FILE, "<init>", "IOException " + iox.getMessage(), Debug.DBG_1);
	...
	void setSource(SmeAddress s)
	{
		if(s != null)
		{
			source = s;
			if(Debug.dbg)
				Debug.d(FILE, "setSource", "Source set to "+s, Debug.DBG_4);
		}
		else if(Debug.dbg)
			Debug.d(FILE, "setSource", "Attempt to set source to null", Debug.DBG_3);
	}
	...
	void setDestination(SmeAddress s)
	{
		if(s != null)
		{
			destination = s;
			if(Debug.dbg)
				Debug.d(FILE, "setDestination", "Dest set to"+s, Debug.DBG_4);
		}
		else
			Debug.d(FILE, "setDestination", "Attempt to set destination to null", Debug.DBG_3);
	}
	...
	void setMessageFlags(MsgFlags f)
	{
		if(f != null)
		{
			flags = f;
			if(Debug.dbg)
				Debug.d(FILE, "setMessageFlags", "Message flags set to "+f, Debug.DBG_4);
		}
		else
			Debug.d(FILE, "setMessageFlags", "Attempt to set flags to null", Debug.DBG_3);
	}
	void setPriority(boolean b)
	{
		...
		flags.priority = b;
		if(Debug.dbg)
			Debug.d(FILE, "setPriority", b, Debug.DBG_4);
	}
	void setRegistered(boolean b)
	{
		...
		flags.registered = b;
		if(Debug.dbg)
			Debug.d(FILE, "setRegistered", b, Debug.DBG_4);
	}
	void setReplaceIfPresent(boolean b)
	{
		...
		if(Debug.dbg)
			Debug.d(FILE, "setReplaceIfPresent", b, Debug.DBG_4);
	}
	void setEsmClass(int c)
	{
		...
		flags.esm_class = c;
		if(Debug.dbg)
			Debug.d(FILE, "setEsmClass", c, Debug.DBG_4);
	}
	void setProtocolId(int id)
	{
		...
		flags.protocol = id;
		if(Debug.dbg)
			Debug.d(FILE, "setProtocol", id, Debug.DBG_4);
	}
	void setDataCoding(int dc)
	{
		...
		if(Debug.dbg)
			Debug.d(FILE, "setDataCoding", dc, Debug.DBG_4);
	}
	void setDefaultMsg(int id)
	{
		...
		if(Debug.dbg)
			Debug.d(FILE, "setDefaultMsg", id, Debug.DBG_4);
	}
	...
	void setMessageText(String text)
	{
		...
		if(text.length() < 161)
		{
			...
			if(Debug.dbg)
				Debug.d(FILE, "setMessageText", text, Debug.DBG_4);
		}
		else
		{
			Debug.d(FILE, "setMessageText", "Message too long", Debug.DBG_1);
	...
	void setServiceType(String type)
	{
		...
		if(type.length() < 6)
		{
			...
			if(Debug.dbg)
				Debug.d(FILE, "setServiceType", type, Debug.DBG_4);
		}
		else
		{
			Debug.d(FILE, "setServiceType", "Service type too long", Debug.DBG_1);
	...
	void setDeliveryTime(Date d)
	{
		...
		deliveryTime = d;
		if(Debug.dbg)
			Debug.d(FILE, "setDeliveryTime", "Delivery time set to "+d, Debug.DBG_4);
	}
	...
	void setExpiryTime(Date d)
	{
		...
		expiryTime = d;
		if(Debug.dbg)
			Debug.d(FILE, "setExpiryTime", "Expiry time set to "+d, Debug.DBG_4);
	}
	Date getExpiryTime()
		{ return expiryTime; }
	...
	void setFinalDate(Date d)
	{
		...
		finalDate = d;
		if(Debug.dbg)
			Debug.d(FILE, "setFinalDate", "Final date set to "+d, Debug.DBG_4);
	}
	Date getFinalDate()
		{ return finalDate; }
	...
	void setMessageId(int id)
	{
		...
		if(sid.length() > 9)
		{
			Debug.d(FILE, "setMessageId", "id invalid"+id, Debug.DBG_1);
			throw new SMPPException("Message Id invalid: range [00000000h - FFFFFFFFh");
		}

		messageId = id;
		if(Debug.dbg)
			Debug.d(FILE, "setMessageId", id, Debug.DBG_4);
	}
	int getMessageId()
		{ return messageId; }
	...
	void setMessageStatus(int st)
	{
		messageStatus = st;
		if(Debug.dbg)
			Debug.d(FILE, "setMessageStatus", st, Debug.DBG_4);
	}
	...
	void setErrorCode(int code)
	{
		errorCode = code; 
		if(Debug.dbg)
			Debug.d(FILE, "setErrorCode", code, Debug.DBG_4);
	}
	int getErrorCode()
		{ return errorCode; }
	...
	public void writeTo(OutputStream out)
	{
		try
		{
			...
			if(Debug.dbg)
				Debug.d(FILE, "writeTo", "Header written to "+out.getClass().getName(), Debug.DBG_4);

		}
		catch(IOException x)
		{
			Debug.d(FILE, "writeTo", "IOException "+x.getMessage(), Debug.DBG_1);
		...
		throws SocketException, IOException
	{
		...
		Debug.d(FILE, "readPacket", "Header(len:"+cmdLen+", id:"+Integer.toHexString(cmdId)+", status:"+cmdSt, Debug.DBG_2);
		...
		{
			case ESME_NACK:
				Debug.d(FILE, "readPacket", "GenericNack", Debug.DBG_3);
				return new GenericNack(bin);
				
			case ESME_BNDRCV:
				Debug.d(FILE, "readPacket", "BindReceiver", Debug.DBG_3);
				return new BindReceiver(bin);

			case ESME_BNDRCV_RESP:
				Debug.d(FILE, "readPacket", "BindReceiverResp", Debug.DBG_3);
				return new BindReceiverResp(bin);

			case ESME_BNDTRN:
				Debug.d(FILE, "readPacket", "BindTransmitter", Debug.DBG_3);
				return new BindTransmitter(bin);

			case ESME_BNDTRN_RESP:
				Debug.d(FILE, "readPacket", "BindTransmitterResp", Debug.DBG_3);
				return new BindTransmitterResp(bin);

			case ESME_UBD:
				Debug.d(FILE, "readPacket", "Unbind", Debug.DBG_3);
				return new Unbind(bin);

			case ESME_UBD_RESP:
				Debug.d(FILE, "readPacket", "UnbindResp", Debug.DBG_3);
				return new UnbindResp(bin);

			case ESME_SUB_SM:
				Debug.d(FILE, "readPacket", "SubmitSM", Debug.DBG_3);
				return new SubmitSM(bin);

			case ESME_SUB_SM_RESP:
				Debug.d(FILE, "readPacket", "SubmitSMResp", Debug.DBG_3);
				return new SubmitSMResp(bin);

			case ESME_SUB_MULTI:
				Debug.d(FILE, "readPacket", "SubmitMulti", Debug.DBG_3);
				return new SubmitMulti(bin);

			case ESME_SUB_MULTI_RESP:
				Debug.d(FILE, "readPacket", "SubmitMultiResp", Debug.DBG_3);
				return new SubmitMultiResp(bin);

			case SMSC_DELIVER_SM:
				Debug.d(FILE, "readPacket", "DeliverSm", Debug.DBG_3);
				return new DeliverSM(bin);

			case SMSC_DELIVER_SM_RESP:
				Debug.d(FILE, "readPacket", "DeliverSMResp", Debug.DBG_3);
				return new DeliverSMResp(bin);

			case ESME_QUERY_SM:
				Debug.d(FILE, "readPacket", "QuerySM", Debug.DBG_3);
				return new QuerySM(bin);

			case ESME_QUERY_SM_RESP:
				Debug.d(FILE, "readPacket", "QuerySMResp", Debug.DBG_3);
				return new QuerySMResp(bin);

			case ESME_QUERY_LAST_MSGS:
				Debug.d(FILE, "readPacket", "QueryLastMsgs", Debug.DBG_3);
				return new QueryLastMsgs(bin);

			case ESME_QUERY_LAST_MSGS_RESP:
				Debug.d(FILE, "readPacket", "QueryLastMsgsResp", Debug.DBG_3);
				return new QueryLastMsgsResp(bin);

			case ESME_QUERY_MSG_DETAILS:
				Debug.d(FILE, "readPacket", "QueryMsgDetails", Debug.DBG_3);
				return new QueryMsgDetails(bin);

			case ESME_QUERY_MSG_DETAILS_RESP:
				Debug.d(FILE, "readPacket", "QueryMsgDetailsResp", Debug.DBG_3);
				return new QueryMsgDetailsResp(bin);
			
			case ESME_CANCEL_SM:
				Debug.d(FILE, "readPacket", "CancelSM", Debug.DBG_3);
				return new CancelSM(bin);

			case ESME_CANCEL_SM_RESP:
				Debug.d(FILE, "readPacket", "CancelSMResp", Debug.DBG_3);
				return new CancelSMResp(bin);

			case ESME_REPLACE_SM:
				Debug.d(FILE, "readPacket", "ReplaceSM", Debug.DBG_3);
				return new ReplaceSM(bin);

			case ESME_REPLACE_SM_RESP:
				Debug.d(FILE, "readPacket", "ReplaceSMResp", Debug.DBG_3);
				return new ReplaceSMResp(bin);

			case ESME_QRYLINK:
				Debug.d(FILE, "readPacket", "EnquireLink", Debug.DBG_3);
				return new EnquireLink(bin);

			case ESME_QRYLINK_RESP:
				Debug.d(FILE, "readPacket", "EnquireLinkResp", Debug.DBG_3);
				return new EnquireLinkResp(bin);

			case ESME_PARAM_RETRIEVE:
				Debug.d(FILE, "readPacket", "ParamRetrieve", Debug.DBG_3);
				return new ParamRetrieve(bin);

			case ESME_PARAM_RETRIEVE_RESP:
				Debug.d(FILE, "readPacket", "ParamRetrieveResp", Debug.DBG_3);
				return new ParamRetrieveResp(bin);

			default:
				Debug.d(FILE, "readPacket", "Unknown Packet", Debug.DBG_3);
				throw new SMPPException("Unidentified Packet on input stream. id=" + String.valueOf(cmdId));
	...
	public static final String getGsmErr(int code)
	{
		...
		if(code == GSM_ERR_NONE)			return new String(GsmStrings[ptr]);
		else ptr++;
		if(code == GSM_ERR_P_UNKNOWN)		return new String(GsmStrings[ptr]);
		else ptr++;
		if(code == GSM_ERR_P_PROVISION)		return new String(GsmStrings[ptr]);
		else ptr++;
		if(code == GSM_ERR_T_BARRED)		return new String(GsmStrings[ptr]);
		else ptr++;
		if(code == GSM_ERR_P_CUG)			return new String(GsmStrings[ptr]);
		else ptr++;
		if(code == GSM_ERR_T_MSSUPPORT)		return new String(GsmStrings[ptr]);
		else ptr++;
		if(code == GSM_ERR_T_MSERROR)		return new String(GsmStrings[ptr]);
		else ptr++;
		if(code == GSM_ERR_T_SUPPORT)		return new String(GsmStrings[ptr]);
		else ptr++;
		if(code == GSM_ERR_T_MEMCAP)		return new String(GsmStrings[ptr]);
		else ptr++;
		if(code == GSM_ERR_T_ABSENT)		return new String(GsmStrings[ptr]);
		else ptr++;
		if(code == GSM_ERR_T_ABSENT_DETACHED)return new String(GsmStrings[ptr]);
		else ptr++;
		if(code == GSM_ERR_T_ABSENT_PAGEFAIL)return new String(GsmStrings[ptr]);
		else ptr++;
		if(code == GSM_ERR_T_SUPPORT_ROAMING)return new String(GsmStrings[ptr]);
		else ptr++;
		if(code == GSM_ERR_T_SYSTEM)		return new String(GsmStrings[ptr]);
		else ptr++;
